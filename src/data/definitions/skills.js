...
export const skills = {
    ...
    weaponryPreparation: { name: "Оружейная Подготовка", description: "Позволяет ковать бронзовые секиры, увеличивая ваше мастерство оружейника.", icon: "gite", cost: { matter: 1050, sparks: 10000, bronzeIngots: 70 }, requires: ["blueprint_fineWeapons"], apply: (state) => {}, relatedItem: 'bronzeGreataxe' },
    repairWorkshop: { name: "Ремонтная Мастерская", description: "Открывает возможность создания комплектов для ремонта брони.", icon: "build", cost: { matter: 950, sparks: 8500, ironIngots: 100 }, requires: ["blueprint_eliteArmor"], apply: (state) => {}, relatedItem: 'armorRepairKit' },
    crossbowMastery: { name: "Мастер Арбалетов", description: "Позволяет создавать арбалеты из искростали.", icon: "sports_martial_arts", cost: { matter: 1300, sparks: 16000, sparksteelIngots: 20 }, requires: ["legendaryClients", "blueprint_fineWeapons"], apply: (state) => {}, relatedItem: 'sparksteelCrossbow' },
    universalPincers: { name: "Универсальные Клещи", description: "Открывает рецепт клещей из искростали.", icon: "handyman", cost: { matter: 1200, sparks: 15000, sparksteelIngots: 15 }, requires: ["optimizedSmelting"], apply: (state) => {}, relatedItem: 'sparksteelPincers' },
